...
unless node['ndb']['restore']['exclude_tables_data'].empty?
    exclude_tables_option = "-e #{node['ndb']['restore']['exclude_tables_data']}"
end
unless node['ndb']['restore']['exclude_databases_data'].empty?
    exclude_databases_option = "-x #{node['ndb']['restore']['exclude_databases_data']}"
end
...
bash 'ndb_restore data' do
    ...
    group 'root'
    code <<-EOH
        #{node['ndb']['scripts_dir']}/restore_backup.sh ndb-restore -p #{backup_directory} -n #{my_node_id} -b #{node['ndb']['restore']['backup_id']} -c #{mgm_connection} #{exclude_tables_option} #{exclude_databases_option} #{exclude_disk_objects_option} -m DATA
    EOH
